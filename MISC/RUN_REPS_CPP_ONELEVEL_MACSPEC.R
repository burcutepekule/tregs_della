...
for (reps_in in 0:(num_reps-1)){
  ...
  phagocyte_activity_ROS = rep(activity_ROS_M0, n_phagocytes)
  phagocyte_activity_engulf = rep(activity_engulf_M0, n_phagocytes)
    ...
    if (length(M0_indices) > 0) {
      ...
      for (idx in seq_along(M0_indices)) {
        ...
        if (avg_DAMPs >= activation_threshold_DAMPs && avg_DAMPs > avg_SAMPs) {
          ...
          phagocyte_activity_ROS[i] = activity_ROS_M1 # + activity_ROS_M1_step * bacteria_count
          phagocyte_activity_engulf[i] = activity_engulf_M1 # + activity_engulf_M1_step * bacteria_count
          ...
          phagocyte_activity_engulf[i] = activity_engulf_M2 # + activity_engulf_M2_step * bacteria_count
        }
      }
    }
